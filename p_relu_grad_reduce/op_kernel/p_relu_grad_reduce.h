...
private:
    ...
    __aicore__ inline float SumRangeFp16(uint32_t start, uint32_t end)
    {
        ...
        uint32_t i = start;
        AscendC::LocalTensor<half> inLocal = inBuf_.Get<half>();
        AscendC::LocalTensor<half> reduceDstLocal = reduceDstBuf_.Get<half>();
        AscendC::LocalTensor<half> reduceTmpLocal = reduceTmpBuf_.Get<half>();
    ...
    __aicore__ inline float SumRangeFp32(uint32_t start, uint32_t end)
    {
        ...
        AscendC::LocalTensor<float> reduceDstLocal = reduceDstBuf_.Get<float>();
        AscendC::LocalTensor<float> reduceTmpLocal = reduceTmpBuf_.Get<float>();
    ...
    __aicore__ inline float SumRangeBf16(uint32_t start, uint32_t end)
    {
        ...
        while (i < end) {
            ...
            for(uint32_t k = 0; k < chunk; ++k) {
                inLocal.SetValue(k, updatesBf16Gm_.GetValue(i + k));
            }
            AscendC::PipeBarrier<PIPE_ALL>();
            for(uint32_t k = 0; k < chunk; ++k) {
                sum += CastBf16ToFloat(inLocal.GetValue(k));
            }
            i += chunk;
        }
        return sum;
