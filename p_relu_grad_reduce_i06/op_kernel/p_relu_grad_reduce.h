...
public:
        ...
        const PReluGradReduceTilingData* tilingData)
    {
        ...
        } else if (reduceMode_ == REDUCE_MODE_NC1HWC0 && reduceSplit_ > 1 && c0_ == DA_ALIGN) {
            __gm__ uint8_t* wsBase = (__gm__ uint8_t*)workspace;
            size_t syncBytes = static_cast<size_t>(blockDim_) * 32;
            syncGm_.SetGlobalBuffer((__gm__ int32_t*)(wsBase + sysWorkspaceSize_), blockDim_ * 8);
            partialSumGm_.SetGlobalBuffer((__gm__ float*)(wsBase + sysWorkspaceSize_ + syncBytes),
                c1_ * reduceSplit_ * c0_);
            pipe_.InitBuffer(syncUbBuf_, blockDim_ * 32);
        }
    ...
    {
        if (reduceMode_ == REDUCE_MODE_SHARED_PARTIAL) {
            ProcessSharedPartial();
            return;
        }

        if (reduceMode_ == REDUCE_MODE_SHARED_FINAL) {
            ProcessSharedFinal();
            return;
        }
        ...
        if (reduceMode_ == REDUCE_MODE_NC1HWC0 && reduceSplit_ > 1 && c0_ == DA_ALIGN) {
            ProcessNc1hwc0ReduceSplit();
            return;
        }
...
private:
    ...
    __aicore__ inline void ProcessSharedPartial()
    {
        uint32_t blockIdx = AscendC::GetBlockIdx();
        uint32_t cn = blockDim_ == 0 ? 1 : blockDim_;
        if (blockIdx >= cn || outputLength_ == 0) return;

        uint32_t elemsPerN = totalLength_ / outputLength_;
        if (elemsPerN == 0) return;

        for (uint32_t nIdx = blockIdx; nIdx < outputLength_; nIdx += cn) {
            uint32_t start = nIdx * elemsPerN;
            uint32_t end = (nIdx == outputLength_ - 1) ? totalLength_ : start + elemsPerN;
            float partial = 0.0F;
            if (dtypeMode_ == DTYPE_MODE_FP32) {
                partial = SumRangeFp32(start, end);
            } else if (dtypeMode_ == DTYPE_MODE_BF16) {
                partial = SumRangeBf16(start, end);
            } else {
                partial = SumRangeFp16(start, end);
            }
            daFloatGm_.SetValue(nIdx, partial);
        }
    }

    __aicore__ inline void ProcessSharedFinal()
    {
        if (AscendC::GetBlockIdx() != 0) return;

        float total = SumRangeFp32(0, totalLength_);
        WriteSharedOutput(total);
    }
    ...
    __aicore__ inline void ClearNc1hwc0C0F32(AscendC::LocalTensor<float> tensor)
    {
        for (uint32_t lane = 0; lane < c0_; ++lane) {
            tensor.SetValue(lane, 0.0F);
        }
        AscendC::PipeBarrier<PIPE_ALL>();
    }

    __aicore__ inline void AccumulateNc1hwc0Span(uint32_t gmOffset, uint32_t vecCount,
        AscendC::LocalTensor<float> accF32)
    {
        uint32_t elemsLeft = vecCount * c0_;
        uint32_t offset = 0;
        while (elemsLeft > 0) {
            uint32_t currentChunk = elemsLeft < V19_UB_REDUCE_ELEMS ? elemsLeft : V19_UB_REDUCE_ELEMS;
            currentChunk = (currentChunk / c0_) * c0_;
            if (currentChunk == 0) break;

            AscendC::LocalTensor<float> calcF32 = reduceTmpBuf_.Get<float>();
            if (dtypeMode_ == DTYPE_MODE_FP32) {
                AscendC::DataCopy(calcF32, updatesFloatGm_[gmOffset + offset], currentChunk);
                AscendC::PipeBarrier<PIPE_ALL>();
            } else if (dtypeMode_ == DTYPE_MODE_FP16) {
                AscendC::LocalTensor<half> inLocal = inBuf_.Get<half>();
                AscendC::DataCopy(inLocal, updatesHalfGm_[gmOffset + offset], currentChunk);
                AscendC::PipeBarrier<PIPE_ALL>();
                AscendC::Cast(calcF32, inLocal, AscendC::RoundMode::CAST_NONE, currentChunk);
                AscendC::PipeBarrier<PIPE_ALL>();
            } else {
                AscendC::LocalTensor<bfloat16_t> inLocal = inBuf_.Get<bfloat16_t>();
                AscendC::DataCopy(inLocal, updatesBf16Gm_[gmOffset + offset], currentChunk);
                AscendC::PipeBarrier<PIPE_ALL>();
                for (uint32_t lane = 0; lane < currentChunk; ++lane) {
                    calcF32.SetValue(lane, CastBf16ToFloat(inLocal.GetValue(lane)));
                }
                AscendC::PipeBarrier<PIPE_ALL>();
            }

            for (uint32_t laneBase = 0; laneBase < currentChunk; laneBase += c0_) {
                AscendC::Add(accF32, accF32, calcF32[laneBase], c0_);
                AscendC::PipeBarrier<PIPE_ALL>();
            }

            elemsLeft -= currentChunk;
            offset += currentChunk;
        }
    }

    __aicore__ inline void WriteNc1hwc0Output(uint32_t c1Idx, AscendC::LocalTensor<float> accF32)
    {
        uint32_t outBase = c1Idx * c0_;
        if (dtypeMode_ == DTYPE_MODE_FP32) {
            AscendC::DataCopy(daFloatGm_[outBase], accF32, c0_);
        } else if (dtypeMode_ == DTYPE_MODE_FP16) {
            AscendC::LocalTensor<half> outLocal = outBuf_.Get<half>();
            AscendC::Cast(outLocal, accF32, AscendC::RoundMode::CAST_NONE, c0_);
            AscendC::PipeBarrier<PIPE_ALL>();
            AscendC::DataCopy(daHalfGm_[outBase], outLocal, c0_);
        } else {
            AscendC::LocalTensor<bfloat16_t> outLocal = outBuf_.Get<bfloat16_t>();
            for (uint32_t lane = 0; lane < c0_; ++lane) {
            ...
            AscendC::PipeBarrier<PIPE_ALL>();
            AscendC::DataCopy(daBf16Gm_[outBase], outLocal, c0_);
        }
        AscendC::PipeBarrier<PIPE_ALL>();
    }

    __aicore__ inline void ProcessNc1hwc0ReduceSplit()
    {
        uint32_t blockIdx = AscendC::GetBlockIdx();
        uint32_t cn = blockDim_ == 0 ? 1 : blockDim_;
        if (blockIdx >= cn) return;
        ...
        uint32_t c1Idx = blockIdx / reduceSplit;
        uint32_t reduceSlice = blockIdx - c1Idx * reduceSplit;
        if (c1Idx >= c1_) return;
        uint32_t hwTotal = h_ * w_;
        uint32_t reduceElems = n_ * hwTotal;
        ...
        ClearNc1hwc0C0F32(accF32);

        uint32_t elem = elemStart;
        while (elem < elemEnd) {
            uint32_t nIdx = elem / hwTotal;
            uint32_t hwIdx = elem - nIdx * hwTotal;
            uint32_t nEnd = (nIdx + 1) * hwTotal;
            uint32_t spanEnd = elemEnd < nEnd ? elemEnd : nEnd;
            uint32_t vecCount = spanEnd - elem;
            uint32_t gmOffset = ((nIdx * c1_ + c1Idx) * hwTotal + hwIdx) * c0_;
            AccumulateNc1hwc0Span(gmOffset, vecCount, accF32);
            elem = spanEnd;
        }

        uint32_t partialBase = (c1Idx * reduceSplit + reduceSlice) * c0_;
        AscendC::DataCopy(partialSumGm_[partialBase], accF32, c0_);
        AscendC::PipeBarrier<PIPE_MTE3>();
        AscendC::LocalTensor<int32_t> ubSync = syncUbBuf_.Get<int32_t>();
        AscendC::SyncAll<true>(syncGm_, ubSync, static_cast<int32_t>(cn));
        ...
        if (reduceSlice == 0) {
            ...
            for (uint32_t slice = 0; slice < reduceSplit; ++slice) {
                uint32_t mergeBase = (c1Idx * reduceSplit + slice) * c0_;
                AscendC::DataCopy(tileF32, partialSumGm_[mergeBase], c0_);
                AscendC::PipeBarrier<PIPE_ALL>();
                AscendC::Add(accF32, accF32, tileF32, c0_);
                AscendC::PipeBarrier<PIPE_ALL>();
            }
            WriteNc1hwc0Output(c1Idx, accF32);
        }
    }
