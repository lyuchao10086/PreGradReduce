...
static bool IsSharedWeight(const aclTensor *weights)
{
    const auto weightsShape = weights->GetViewShape();
    return weightsShape.GetDimNum() == 1 && weightsShape.GetDim(0) == 1;
}
    ...
    aclOpExecutor  **executor)
{
    ...
    auto daTemp = IsSharedWeight(weightsContiguous) ?
        l0op::PReluGradReduceShared(updatesContiguous, weightsContiguous, uniqueExecutor.get()) :
        l0op::PReluGradReduce(updatesContiguous, weightsContiguous, uniqueExecutor.get());
