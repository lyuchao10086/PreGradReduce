...
const std::initializer_list<ge::DataType> PARTIAL_DTYPES = {ge::DT_FLOAT};
const std::initializer_list<ge::Format> PARTIAL_FORMATS = {ge::FORMAT_ND};
...
class PReluGradReducePartial : public OpDef {
public:
    explicit PReluGradReducePartial(const char* name) : OpDef(name)
    {
        this->Input("grads")
            .ParamType(REQUIRED)
            .DataType(GRAD_DTYPES)
        ...
        this->Output("partial")
            .ParamType(REQUIRED)
            .DataType(PARTIAL_DTYPES)
            .Format(PARTIAL_FORMATS)
            .UnknownShapeFormat(PARTIAL_FORMATS)
            .AutoContiguous();
        ...
        aicoreConfig.DynamicCompileStaticFlag(true)
            ...
            .PrecisionReduceFlag(true)
            .ExtendCfgInfo("opFile.value", "p_relu_grad_reduce")
            .ExtendCfgInfo("opInterface.value", "p_relu_grad_reduce_partial");
        this->AICore().AddConfig("ascend910b", aicoreConfig);
        this->AICore().AddConfig("ascend910_93", aicoreConfig);
        this->AICore().AddConfig("ascend950", aicoreConfig);
    }
...
class PReluGradReduceFinal : public OpDef {
public:
    explicit PReluGradReduceFinal(const char* name) : OpDef(name)
    {
        this->Input("grads")
            .ParamType(REQUIRED)
            .DataType(PARTIAL_DTYPES)
        ...
        aicoreConfig.DynamicCompileStaticFlag(true)
            ...
            .PrecisionReduceFlag(true)
            .ExtendCfgInfo("opFile.value", "p_relu_grad_reduce")
            .ExtendCfgInfo("opInterface.value", "p_relu_grad_reduce_final");
        this->AICore().AddConfig("ascend910b", aicoreConfig);
        this->AICore().AddConfig("ascend910_93", aicoreConfig);
        this->AICore().AddConfig("ascend950", aicoreConfig);
    }
...
OP_ADD(PReluGradReducePartial);
OP_ADD(PReluGradReduceFinal);
} // namespace ops
