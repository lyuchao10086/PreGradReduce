...
bool IsSharedWeightCase(const CaseSpec& testCase)
{
    return testCase.weightsShape.size() == 1 && testCase.weightsShape[0] == 1;
}
    ...
    const std::vector<T>& expectedHostData, float tolerance, bool isBf16 = false)
{
    ...
    if (IsBenchmarkEnabled() && testCase.benchmark) {
        ...
        for (uint32_t i = 0; i < warmup; ++i) {
            ret = IsSharedWeightCase(testCase) ?
                RunPReluGradReduceByAclnn(gradsBuffer, featuresBuffer, weightsBuffer, updatesBuffer, daBuffer,
                    testCase, stream) :
                RunPReluGradReduceByAclop(gradsBuffer, featuresBuffer, weightsBuffer, updatesBuffer, daBuffer,
                    testCase, stream);
            CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("benchmark warmup failed. ERROR: %d\n", ret);
                return ret);
        }
        ret = aclrtSynchronizeStream(stream);
        ...
        for (uint32_t i = 0; i < iterations; ++i) {
            ret = IsSharedWeightCase(testCase) ?
                ...
                RunPReluGradReduceByAclop(gradsBuffer, featuresBuffer, weightsBuffer, updatesBuffer, daBuffer,
                    testCase, stream);
            CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("benchmark execute failed. ERROR: %d\n", ret);
                return ret);
        }
...
int PReluGradReduceTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    LOG_PRINT("[SMOKE] case_count=%zu run_count=%u dtypes=fp32,fp16,bf16 formats=ND,NC1HWC0 "
              "axis_generalization=rank2,rank3,rank4 shared_large_cases=two_stage "
